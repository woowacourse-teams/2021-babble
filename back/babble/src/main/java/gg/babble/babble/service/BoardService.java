...
public class BoardService {
    ...
    public BoardSearchResponse search(final String type, final String keyword) {
        BoardSearchType boardSearchType = BoardSearchType.from(type);

        List<Board> boards = boardSearchType.compose(boardRepository, keyword);

        List<BoardResponse> result = boards.stream()
            .map(BoardResponse::from)
            .collect(Collectors.toList());

        return new BoardSearchResponse(result, keyword, type);
    }
    ...
    public List<BoardResponse> findByCategory(final String name) {
